...
>(({ className, asChild = false, rounded = false, ...props }, ref) => {
  return (
    <button
      className={cn(
        `group relative inline-flex items-center gap-2 ${rounded ? "aspect-square rounded-full p-3" : "rounded px-6 py-4"} bg-foreground font-bold leading-none text-background outline-hidden ring-primary ring-offset-4 ring-offset-background focus:ring-2`,
        className,
      )}
      ref={ref}
      {...props}
